...
if (isset($_POST['add_room'])) 
{
    ...
    if (insert($q1, $values, 'ssiiiii'))
    {
        $flag = 1;
        $room_id = mysqli_insert_id($con);

            // Room Facilities insert
            $q2 = "INSERT INTO `room_facilities`(`room_id`, `facilities_id`) VALUES (?,?)";
            if ($stmt = mysqli_prepare($con, $q2))
            {
                foreach ($facilities as $f1)
                {
                    mysqli_stmt_bind_param($stmt, "ii", $room_id, $f1);
...
if (isset($_POST['get_all_rooms'])) 
{
    ...
    while ($row = mysqli_fetch_assoc($res))
    {
        if($row['status']==1)
        {
            $status = "<button onclick='toggleStatus($row[id],0)' class='btn btn-dark btm-sm shadow-none'>Active</button>";
        }
        else
        {
            $status= "<button onclick='toggleStatus($row[id],1)' class='btn btn-warning btm-sm shadow-none'>In-Active</button>";
        }
        ...
        $data .= "
            <tr class='align-middle'>
                ...
                <button type='button' onclick='remove_room($row[id])' class='btn btn-danger text-white fw-bold shadow-none btn-sm mb-1'>
...
if ($stmt = mysqli_prepare($con, $q2))
{
    foreach ($facilities as $f1)
    {
        mysqli_stmt_bind_param($stmt, "ii", $frm_data['room_id'], $f1); // Removed $ before room_id
        mysqli_stmt_execute($stmt);
    }
    $flag = 1; // Moved this line inside the loop
    mysqli_stmt_close($stmt);
}
else
{
...
if ($stmt = mysqli_prepare($con, $q3))  
{
    foreach ($features as $f)
    {
        mysqli_stmt_bind_param($stmt, "ii", $frm_data['room_id'], $f); // Removed $ before room_id
        mysqli_stmt_execute($stmt);
    }
    $flag = 1; // Moved this line inside the loop
    mysqli_stmt_close($stmt);
    ...
    while ($row = mysqli_fetch_assoc($res)) 
        {
            if($row['thumb']==1)
            {
                $thumb_btn = "<i class='bi bi-check-lg rounded text-light bg-success px-2 py-1 fs-5'></i>";
            }
            else
            {
                $thumb_btn = "
                    <button onclick='thumb_image($row[sr_no],$row[room_id])' class='btn btn-secondary shadow rounded text-white '>
            ...
            echo <<<data
                <tr class = 'align-middle'>
                    ...
                    <td>$thumb_btn</td>
                    <td>
                        <button onclick='rem_image($row[sr_no],$row[room_id])' class='btn btn-danger shadow rounded text-white '>
                            <i class='bi bi-trash'></i>
                        </button>                  
                    </td>
                </tr>
